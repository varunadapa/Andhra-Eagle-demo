...
function navigate(page, subData) {
  ...
  $('#page-title').textContent = titles[page] || 'NOTS';
...
function renderDashboard() {
  ...
  html += `<div class="card">
    <div class="card-header">
      ...
      <a href="#" style="font-size:0.8rem;color:var(--accent-green);text-decoration:none;font-weight:600">View All →</a>
    </div>
  ...
  html += `<div class="card">
    <div class="card-header">
      ...
      <a href="#" style="font-size:0.8rem;color:var(--accent-green);text-decoration:none;font-weight:600">Details →</a>
    </div>
  ...
  chartInstances.firs = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: 'rgba(189, 157, 30, 0.7)', /* Gold */
        borderColor: '#bd9d1e',
...
function renderDrugChart(data) {
  ...
  chartInstances.drugs = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { label: 'Worth (₹ Cr)', data: data.map(d => d.worth), backgroundColor: 'rgba(189, 157, 30, 0.8)', borderColor: '#bd9d1e', borderWidth: 2, borderRadius: 4, barPercentage: 0.5 }, /* Gold */
        { label: 'Cases', data: data.map(d => d.cases), backgroundColor: 'rgba(218, 165, 32, 0.8)', borderColor: '#daa520', borderWidth: 2, borderRadius: 4, barPercentage: 0.5 } /* Slightly lighter gold for secondary bar */
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
...
function renderInterrogations() {
  ...
  let html = '<div class="card"><div class="card-header"><div class="card-title"><i class="fas fa-clipboard-list" style="color:var(--accent-green);margin-right:8px"></i>Interrogation Reports</div><div class="card-subtitle">Comprehensive field interrogation and modus operandi records</div></div>'
...
function renderChargesheets() {
  ...
  let html = '<div class="card"><div class="card-header"><div class="card-title"><i class="fas fa-gavel" style="color:var(--accent-green);margin-right:8px"></i>Chargesheets Filed</div><div class="card-subtitle">Court filing records and disposal tracking</div></div>'
    + '<div class="data-table-wrapper"><table class="data-table"><thead><tr>'
...
function rendernotsAI() {
  ...
  html += `
    <div class="ai-bottom-section">
      ${!hasHistory ? `
        <div class="ai-hero">
          <div style="width: 100px; height: 100px; margin: 0 auto 24px auto; background-color: #ffffff; background-image: url('/assets/logo.png'); background-size: contain; background-repeat: no-repeat; background-position: center; background-origin: content-box; border-radius: 50%; padding: 10px; box-shadow: 0 8px 30px rgba(0,0,0,0.15); flex-shrink: 0;"></div>
          <h2>Meet NOTS, your intelligence assistant</h2>
        </div>
      ` : ''}

      <div class="ai-input-pill">
        <i class="fas fa-plus action-icon"></i>
        <input type="text" id="ai-input" placeholder="Ask NOTS..." autocomplete="off" onkeypress="if(event.key==='Enter') sendChatMessage()" />
